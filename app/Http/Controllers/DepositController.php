...
class DepositController extends Controller
{
    ...
    public function approve(Deposit $deposit)
    {
        ...
        if ($deposit->status === 'approved') {
            return redirect()->back()->with('error', 'Giao dịch này đã được duyệt trước đó.');
        }
        ...
        // Debug info
        $beforeCoins = $deposit->user->coins;
        ...
        try {
            ...
            // Cộng xu cho người dùng - Load fresh to avoid stale data
            $user = $deposit->user->fresh();
            $user->coins = $user->coins + $deposit->coins;
            $user->save();
            
            $afterCoins = $user->coins;
            
            // Log coin update
            Log::info('Deposit approval', [
                'deposit_id' => $deposit->id,
                'user_id' => $user->id,
                'coins_before' => $beforeCoins,
                'coins_after' => $afterCoins,
                'coins_added' => $deposit->coins
            ]);

            DB::commit();

            return redirect()->back()->with('success', 'Đã phê duyệt giao dịch và cộng ' . $deposit->coins . ' xu vào tài khoản người dùng. (Xu trước: ' . $beforeCoins . ', Xu sau: ' . $afterCoins . ')');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function reject(Request $request, Deposit $deposit)
    {
        ...
        // Check if the deposit is already rejected
        if ($deposit->status === 'rejected') {
            return redirect()->back()->with('error', 'Giao dịch này đã được từ chối trước đó.');
        }
    ...
    public function adminIndex(Request $request)
    {
        ...
        $query = Deposit::with(['user', 'bank', 'approver']);
        ...
        // Tìm kiếm theo mã giao dịch hoặc thông tin người dùng
